...
function SkyPlusHDBox(options) {
   ...
   this.action = function action(opt) {

      var soapPayload = { InstanceID: opt[0], Speed: opt[1] }, deferred = q.defer();
   ...
   this.change = function change(num) {
   ...
   this.play = function play() {
      return _this.action([0, 1, 'Play']);
   };

   this.pause = function pause() {
      return _this.action([0, 1, 'Pause']);
   };

   this.fwd = function fwd() {
      return _this.action([0, 12, 'Play']);
   };
