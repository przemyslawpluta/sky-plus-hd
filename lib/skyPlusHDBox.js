...
function SkyPlusHDBox(options) {
   ...
   this.action = function action(deferred, opt) {

      var soapPayload = { InstanceID: opt[0], Speed: opt[1] };

      _this.soapRequest(settings.Services.SkyPlay, opt[2], soapPayload).fail(function fail(err) {
         deferred.reject(err);
      }).then(function resolve() {
         deferred.resolve();
      }).done();
   ...
   this.change = function change(deferred, num) {
   ...
   this.play = function play() {
      return _this.action(q.defer(), [0, 1, 'Play']);
   };

   this.pause = function pause() {
      return _this.action(q.defer(), [0, 1, 'Pause']);
   };

   this.fwd = function fwd() {
      return _this.action(q.defer(), [0, 12, 'Play']);
   };
