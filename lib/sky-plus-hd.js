...
module.exports = function SkyPlusHD() {
   ...
   this.find = function SkyPlusHDFind() {

      var deferred = q.defer();

      q.all([
         discoverService(SkyPlusHDSettings.Services.SkyRC),
         discoverService(SkyPlusHDSettings.Services.SkyBrowse)
      ]).then(function resolve(vals) {
         ...
         skyBox.on('init', function init() { deferred.resolve(skyBox); });
   ...
   function discoverService(serviceUrn) {
      ...
      var deferred = q.defer(), ssdpClient = new ssdp();

      ssdpClient.on('response', function response(msg, rinfo) {
      ...
      var timeoutTimer = setTimeout(function timeout() {
   ...
   function parseMsg(msg) {
      ...
      _.each(_.str.lines(msg), function each(line) {
